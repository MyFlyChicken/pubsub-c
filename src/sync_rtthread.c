...
int mutex_init(mutex_t *_m) {
	struct rt_mutex *m = rt_malloc(sizeof(struct rt_mutex));
	*_m = m;
...
int semaphore_init(semaphore_t *_s, unsigned int value) {
	struct rt_semaphore *s = rt_malloc(sizeof(struct rt_semaphore));
	*_s = s;
	return rt_sem_init(s, "ps_sem", value, RT_IPC_FLAG_FIFO);
}
